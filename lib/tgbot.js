...
function TgBot(options) {
    ...
    /* Getters & setters */
    this.getOwnIdentity = function() { return identity; };
        ...
        var reqCallback = function(error, response, body) {
            if (error || response.statusCode != 200 || !body.ok || !body.result) {
                ...
                log.error('API call ' + method + ' failed:', {
                    parameters: errorData.callTrace.parameters,
    ...
    function handleUpdate(update, date) {
        ...
        if (update.message) {
            ...
            self.emit('messageReceived', extend(updateData, {
                raw: message,
    ...
    function parseCommand(message) {
        ...
        var command = {
            name: tokens[0].substr(offset),
            argumentTokens: tokens.slice(1)
        };
        command.argument = command.argumentTokens.join(' ');
        return command;
    }
}
