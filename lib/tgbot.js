...
function TgBot(options) {
    ...
    this.api = {
        ...
        sendAudio: function(parameters, options, cb) {
            var callParameters = _.pick(parameters, [
                'chat_id', 'audio', 'duration', 'performer', 'title',
        ...
        sendVoice: function(parameters, options, cb) {
            var callParameters = _.pick(parameters, [
                'chat_id', 'voice', 'duration', 'disable_notification',
                'reply_to_message_id'
            ]);
            addSerializedProperty('reply_markup', callParameters, parameters);
            var callOptions = _.pick(options, 'fileUpload');
            return callApi('sendVoice', callParameters, callOptions)
                .nodeify(cb);
        },
